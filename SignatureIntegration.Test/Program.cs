...
namespace SignatureIntegration.Test
{
	static class Program
	{
		static void Main(string[] args)
		{
            ...
            ISignatureClientForV6 client = new SignatureClientForV6(uri, endpoints);
            ...
            try
            {
                ...
                var cert = client.GetCertificates(login, orgaid, token).FirstOrDefault();
            ...
            JMP:
                ...
                filepades40 = jObj["data"].ToString();
                jObj["data"] = "[DATOS]";
                Console.WriteLine(jObj.ToString());
                Console.WriteLine();
                ...
                filepades60 = jObj["data"].ToString();
                jObj["data"] = "[DATOS]";
                Console.WriteLine(jObj.ToString());
                Console.WriteLine();
                ...
                File.WriteAllBytes(Path.Combine(outpdir, "example_pades_signed.pdf"), Convert.FromBase64String(filepades));
                File.WriteAllBytes(Path.Combine(outpdir, "example_pades_signed_40.pdf"), Convert.FromBase64String(filepades40));
                File.WriteAllBytes(Path.Combine(outpdir, "example_pades_signed_60.pdf"), Convert.FromBase64String(filepades60));
                File.WriteAllBytes(Path.Combine(outpdir, "example_xades_signed.xml"), Convert.FromBase64String(filexades));
                File.WriteAllBytes(Path.Combine(outpdir, "example_cades_signed.p7m"), Convert.FromBase64String(filecades));
